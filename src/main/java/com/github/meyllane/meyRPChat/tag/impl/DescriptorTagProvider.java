...
public class DescriptorTagProvider extends TagProvider {
    public DescriptorTagProvider(String id) {
        super(id);
    }
    ...
    public void registerTag(TagResolver.Builder builder, ChatContext ctx) {
        if (ctx instanceof RangedChatContext rctx) {
            builder.tag(this.id, Tag.inserting(Component.text(rctx.getRange().descriptor())));
        }
    }
}
